...
<body>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="number">{{ $assessment['complete_attendance'] }}</td>
                <td class="number">{{ $assessment['complete_attendance_rate'] }}%</td>
            </tr>
            <tr>
                <td>With absences</td>
                <td class="number">{{ $assessment['with_absences'] }}</td>
                <td class="number">{{ $assessment['with_absences_rate'] }}%</td>
            </tr>
        </tbody>
    </table>
    ...
    <table>
        ...
        <tbody>
            <tr>
                <td>Baseline at-risk nutrition cohort</td>
                <td class="number">{{ $assessment['malnourished'] }}</td>
                <td class="number">100%</td>
            </tr>
            <tr>
                <td>Recovered to Normal at endline</td>
                <td class="number">{{ $assessment['recovered'] }}</td>
                <td class="number">{{ $assessment['recovered_rate'] }}%</td>
            </tr>
            <tr>
                <td>Still needing support at endline</td>
                <td class="number">{{ $assessment['still_needing_support'] }}</td>
                <td class="number">{{ $assessment['still_needing_support_rate'] }}%</td>
            </tr>
        </tbody>
    </table>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                <td>{{ $row['age'] }}</td>
                <td>{{ $row['count'] }}</td>
                <td>{{ $row['complete'] }} ({{ $row['attendance_rate'] }}%)</td>
                <td>{{ $row['with_absences'] }} ({{ $row['absence_rate'] }}%)</td>
            </tr>
            @empty
            <tr>
            ...
            <tr>
                ...
                <td><strong>{{ $sexTotal }}</strong></td>
                <td><strong>{{ $sexComplete }} ({{ $sexTotal ? round($sexComplete / $sexTotal * 100, 1) : 0 }}%)</strong></td>
                <td><strong>{{ $sexAbsences }} ({{ $sexTotal ? round($sexAbsences / $sexTotal * 100, 1) : 0 }}%)</strong></td>
            </tr>
        </tbody>
    </table>
    @endforeach
    ...
    <table>
        <thead>
            <tr>
                <th>Age</th>
                <th>Total Count</th>
                <th>{{ $key === 'participant_demographics' ? 'Participants (%)' : 'Recovered (%)' }}</th>
                <th>{{ $key === 'recovery_demographics' ? 'Still Needing Support (%)' : '' }}</th>
            </tr>
        </thead>
        <tbody>
            @forelse($sexRows as $row)
            <tr>
                <td>{{ $row['age'] }}</td>
                <td>{{ $row['count'] }}</td>
                <td>{{ $key === 'participant_demographics' ? $row['count'] . ' (' . ($sexTotal ? round($row['count'] / $sexTotal * 100, 1) : 0) . '%)' : $row['recovered'] . ' (' . ($row['count'] ? round($row['recovered'] / $row['count'] * 100, 1) : 0) . '%)' }}</td>@if($key === 'recovery_demographics')<td>{{ $row['still_needing_support'] }} ({{ $row['count'] ? round($row['still_needing_support'] / $row['count'] * 100, 1) : 0 }}%)</td>@endif
            </tr>
            @empty
            <tr>
            ...
            <tr>
                <td><strong>Total</strong></td>
                <td><strong>{{ $sexTotal }}</strong></td>
                <td><strong>{{ $key === 'participant_demographics' ? $sexTotal . ' (100%)' : $recovered . ' (' . ($sexTotal ? round($recovered / $sexTotal * 100, 1) : 0) . '%)' }}</strong></td>@if($key === 'recovery_demographics')<td><strong>{{ $support }} ({{ $sexTotal ? round($support / $sexTotal * 100, 1) : 0 }}%)</strong></td>@endif
            </tr>
        </tbody>
    </table>
    @endforeach
