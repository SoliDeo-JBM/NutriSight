...
<style>
    ...
    .attendance-grid .number-column {
        width: 2.75rem;
    }

    .attendance-grid .name-column {
        width: 14rem;
        text-align: left;
        border-right: 2px solid #172033;
    }

    .attendance-grid .grade-column {
        width: 5.5rem;
    }

    .attendance-grid .section-column {
        width: 5rem;
    }

    .attendance-grid .day-column {
        width: 2.65rem;
    }

    .attendance-grid .day-head,
    .attendance-grid .title-head {
        height: 2.25rem;
        border: 1px solid #475569;
        background: #1e293b;
        color: #fff;
    ...
    .attendance-grid .present {
        color: #15803d;
        font-weight: 800;
    }

    .attendance-grid .absent {
        color: #dc2626;
        font-weight: 700;
    }

    .attendance-grid .total-row td {
...
<div class="flex flex-col gap-5">
    ...
    <form method="GET" class="no-print flex flex-wrap items-end gap-3 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <div><label class="mb-1 block text-xs font-semibold text-slate-700">Section</label><select name="section" onchange="this.form.submit()" class="rounded-lg border border-slate-200 px-3 py-2 text-sm">
                <option value="">All sections</option>@foreach($sections as $sectionOption)<option value="{{ $sectionOption }}" {{ $selectedSection === $sectionOption ? 'selected' : '' }}>{{ $sectionOption }}</option>@endforeach
            </select></div>
        <div><label class="mb-1 block text-xs font-semibold text-slate-700">Sort</label><select id="attendanceSort" class="rounded-lg border border-slate-200 px-3 py-2 text-sm">
    ...
    <section class="attendance-sheet overflow-x-auto rounded-xl border border-slate-300 bg-white p-3 shadow-sm">
        <table id="attendanceTable" class="attendance-grid w-full">
            <thead>
                <tr>
                    <th rowspan="2" class="title-head number-column">No.</th>
                    <th rowspan="2" class="title-head name-column">NAME OF PUPIL</th>
                    <th rowspan="2" class="title-head grade-column">GRADE LEVEL</th>
                    <th rowspan="2" class="title-head section-column">SECTION</th>
                    <th colspan="{{ $daysInMonth }}" class="title-head">ACTUAL FEEDING</th>
                </tr>
                <tr>@foreach(range(1, $daysInMonth) as $day)<th class="day-column day-head">{{ $day }}</th>@endforeach</tr>
            </thead>
            <tbody>
                @forelse($students as $student)
                <tr data-name="{{ strtolower($student['name']) }}" data-grade="{{ $student['grade_level'] }}" data-section="{{ strtolower($student['section']) }}">
                    <td class="number-column">{{ $student['number'] }}</td>
                    <td class="name-column font-semibold text-slate-800">{{ $student['name'] }}</td>
                    <td class="grade-column">{{ $student['grade_level'] === 0 ? 'Kinder' : ($student['grade_level'] === 7 ? 'SPED' : 'Grade ' . $student['grade_level']) }}</td>
                    <td class="section-column">{{ $student['section'] }}</td>@foreach($student['days'] as $status)@php($present = in_array(strtolower((string) $status), ['present', 'p', 'served']))<td class="day-column {{ $present ? 'present' : (in_array(strtolower((string) $status), ['absent', 'a']) ? 'absent' : '') }}">{{ $present ? '✓' : (in_array(strtolower((string) $status), ['absent', 'a']) ? 'A' : '') }}</td>@endforeach
                </tr>
                @empty
                <tr>
                    <td colspan="{{ 4 + $daysInMonth }}" class="py-10 text-center text-sm text-slate-500">No enrolled pupils found.</td>
                </tr>
                @endforelse
                <tr class="total-row">
                    <td colspan="4" class="name-column text-right">TOTAL:</td>@foreach(range(1, $daysInMonth) as $day)<td>{{ $students->filter(fn ($student) => in_array(strtolower((string) ($student['days'][$day] ?? '')), ['present', 'p', 'served']))->count() ?: '' }}</td>@endforeach
                </tr>
            </tbody>
        </table>
    </section>
...
<script>
    document.getElementById('attendanceSort')?.addEventListener('change', (event) => {
        const body = document.querySelector('#attendanceTable tbody');
        const total = body.querySelector('.total-row');
        const rows = [...body.querySelectorAll('tr[data-name]')];
        const key = event.target.value;
        rows.sort((first, second) => first.dataset[key].localeCompare(second.dataset[key], undefined, {
            numeric: key === 'grade'
        }));
