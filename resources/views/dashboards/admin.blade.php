...
@section('content')
    ...
    <div class="mb-8">
        <div class="mb-4">
            ...
            <p class="mt-1 text-sm text-gray-500">Active school year results for students with recorded SBFP attendance and nutrition assessments.</p>
        </div>
        <div class="grid grid-cols-1 xl:grid-cols-2 gap-6">
            <div class="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
                <div class="flex items-start justify-between gap-4 mb-5">
                    <div>
                        <h3 class="text-lg font-bold text-gray-800">Student Attendance Summary</h3>
                        <p class="text-xs text-gray-500 mt-1">{{ $attendanceStudentCount }} students with attendance records</p>
                    </div>
                    <div class="text-right">
                        <div class="text-2xl font-bold text-blue-700">{{ $completeAttendanceRate }}%</div>
                        <div class="text-xs text-gray-500">complete attendance</div>
                    </div>
                </div>
                <div class="h-3 w-full overflow-hidden rounded-full bg-rose-100" role="img" aria-label="{{ $completeAttendanceRate }} percent complete attendance and {{ $attendanceWithAbsencesRate }} percent with absences">
                    <div class="h-full bg-emerald-500" style="width: {{ $completeAttendanceRate }}%"></div>
                </div>
                <div class="mt-5 grid grid-cols-2 gap-4">
                    <div class="border-l-4 border-emerald-500 pl-3">
                        <div class="text-2xl font-bold text-gray-900">{{ $completeAttendanceCount }}</div>
                        <div class="text-xs font-semibold uppercase tracking-wide text-gray-500">Complete attendance</div>
                        <div class="text-sm text-emerald-700">{{ $completeAttendanceRate }}%</div>
                    </div>
                    <div class="border-l-4 border-rose-500 pl-3">
                        <div class="text-2xl font-bold text-gray-900">{{ $attendanceWithAbsencesCount }}</div>
                        <div class="text-xs font-semibold uppercase tracking-wide text-gray-500">With absences</div>
                        <div class="text-sm text-rose-700">{{ $attendanceWithAbsencesRate }}%</div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
                <div class="flex items-start justify-between gap-4 mb-5">
                    <div>
                        <h3 class="text-lg font-bold text-gray-800">SBFP Nutrition Progress</h3>
                        <p class="text-xs text-gray-500 mt-1">Term 1 Wasted or Severely Wasted cohort</p>
                    </div>
                    <div class="text-right">
                        <div class="text-2xl font-bold text-emerald-700">{{ $recoveryRate }}%</div>
                        <div class="text-xs text-gray-500">recovered to Normal</div>
                    </div>
                </div>
                <div class="h-3 w-full overflow-hidden rounded-full bg-amber-100" role="img" aria-label="{{ $recoveryRate }} percent recovered to Normal and {{ $stillNeedingSupportRate }} percent still needing support">
                    <div class="h-full bg-emerald-500" style="width: {{ $recoveryRate }}%"></div>
                </div>
                <div class="mt-5 grid grid-cols-2 gap-4">
                    <div class="border-l-4 border-emerald-500 pl-3">
                        <div class="text-2xl font-bold text-gray-900">{{ $recoveredCount }}</div>
                        <div class="text-xs font-semibold uppercase tracking-wide text-gray-500">Recovered to Normal</div>
                        <div class="text-sm text-emerald-700">{{ $recoveryRate }}%</div>
                    </div>
                    <div class="border-l-4 border-amber-500 pl-3">
                        <div class="text-2xl font-bold text-gray-900">{{ $stillNeedingSupportCount }}</div>
                        <div class="text-xs font-semibold uppercase tracking-wide text-gray-500">Still needing support</div>
                        <div class="text-sm text-amber-700">{{ $stillNeedingSupportRate }}%</div>
                    </div>
                </div>
            </div>
        </div>
